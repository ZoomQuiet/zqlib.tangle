...
class CodeReview(object):
    ...
    def is_existent_rev(self):
        self.cursor.execute("SELECT rev FROM revision WHERE rev=%s", (self.id, ))
    ...
    def get_current_ver(self):
        self.cursor.execute("SELECT version FROM review_current WHERE id=%s", (self.id, ))
...
class CommitPath(object):
    ...
    def get_path(self, rev):
        self.cursor.execute("SELECT path FROM rev_path WHERE rev=%s", (rev,))
